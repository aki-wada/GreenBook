...
Finder で `GreenBook_RAG.command` をダブルクリックするだけで起動できます。

1. `greenbook-rag/GreenBook_RAG.command` をダブルクリック
2. ターミナルが開き、モデルの読み込みが始まる
3. 起動完了するとブラウザが自動で開く

> 既に起動中の場合はブラウザだけ開きます。
> 終了するにはターミナルウィンドウを閉じるか、Ctrl+C を押します。
